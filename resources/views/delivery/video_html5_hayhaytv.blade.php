...
function prerollCompleteYomedia() {
    setTimeout("removeVideoInlineYomedia(playerId{!! $data['zid'] !!}, el{!! $data['zid'] !!})", 500);
}
...
function loadAds{!! $data['zid'] !!}() {	
	if ('' != playerId{!! $data['zid'] !!}) {
		...
		if (domPlayerInner) {
        	...
        	var domWrapPlayer = domManipulate.create('div', el{!! $data['zid'] !!}, 'position:relative;top:0;bottom:0;;width:' + elWidth{!! $data['zid'] !!} + 'px;height:' + elHeight{!! $data['zid'] !!} + 'px;');
        	...
        	<?php 
                ...
                $template_video_url = STATIC_URL. "public/source/videojs/ova_html.html";

                if (!empty($data['ad']->source_url_backup)) {
                    $template_video_url = $data['ad']->source_url_backup;
                }
        	?>
