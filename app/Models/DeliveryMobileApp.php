...
class DeliveryMobileApp extends Eloquent{
	 ...
	 */
	public function deliveryAd($ad, $flightWebsite, $flight, $dateRange, $ip_address='', $device_type='', $operating_system='', $user_agent=''){
		//kiểm tra limit inventory
		$deliveredAdStatus = $this->deliveredAdStatus($ad->id, $flightWebsite, $flight, $dateRange, $ip_address, $device_type, $operating_system, $user_agent);
	 ...
	 * return boolean
	 */
	public function checkGeo($listCountries, $listProvinces, $ip_address){
		if(empty($listCountries) || isLocal()){
			return true;
		}

		$retval = false;		
        $geoip = GeoBaseModel::getGeoByIp($ip_address);
        pr($geoip);
	    if($geoip){
	...
	public function deliveredAdStatus($adID, $flightWebsite, $flight, $dateRange, $ip_address='', $device_type='', $operating_system='', $user_agent=''){
		$trackingModel = new Tracking;
		//kiểm tra độ tuổi
		...
		if($flight->country && $flight->province && !$this->checkGeo($flight->country,$flight->province, $ip_address) ){
	...
	public function checkPlatform($ad, $platform) {
	...
	public function frequencyCapping($flightDate) {
		$now = strtotime(date('Y-m-d H:i'));
		$rs['frequency_cap'] = 0;
		$rs['frequency_cap_time'] = 0;
		foreach ($flightDate as $date) {
			if(strtotime($date->start) <= $now && strtotime($date->end) >= $now){
    			$rs['frequency_cap'] = $date->frequency_cap;
    			$rs['frequency_cap_time'] = $date->frequency_cap_time;
