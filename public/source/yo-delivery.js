...
var avlConfig = new function(){
	var private = {
		'CSS': avlProtocal + avlDomain + '/public/source/css/advStyle.css',
		'JW': avlProtocal + avlDomain + '/public/source/flash/jwplayer5/player.swf',
		'EI': avlProtocal + avlDomain + '/public/source/flash/expressInstall.swf',
		'ICW': 'avlContent',
		'IC': '',
		'OI': 'avlOverlayInner',
		'VI': 'avlVideoInner',
...
var avlHelperModule  = new function(){
	...
	return {
		...
		validateUrl: function(varObj){
			var myRegExp =/^(?:(?:http?):\/\/)(?:\S+(?::\S*)?@)?(?:(?!10(?:\.\d{1,3}){3})(?!127(?:\.\d{1,3}){3})(?!169\.254(?:\.\d{1,3}){2})(?!192\.168(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]+-?)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]+-?)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/[^\s]*)?$/i;
        ...
        rotatorPercentAd: function(c, a, p, b) {
            ...
            if (e.length > 0) {
                if (b != '') {
                    ...
                    document.write('<div id="' + b + '">' + e[d] + '</div>')
...
var avlDeliveryModule = new function(){
	return {
		...
		init: function(varObj){
            ...
            return avlHelperModule.embedJs(link);
		}
	} 
		...
		init : function(initVar){
			if (typeof initVar !== 'undefined' && navigator.cookieEnabled) {
			    if (initVar.length > 0) {
				    ...
				    var fpid = '';
				    if (!isNaN(initVar[lastIndexInitVar][2]) && initVar[lastIndexInitVar][2] == parseInt(initVar[lastIndexInitVar][2])) {
				    	fpid = initVar[lastIndexInitVar][2];
				    }
					var varObj 				= {
						"fpid": fpid,
						"wid": initVar[lastIndexInitVar][0],
